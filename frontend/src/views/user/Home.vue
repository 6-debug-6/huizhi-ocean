<template>
  <div class="home-page">
    ...
    <main class="home-main">
      ...
      <!-- 最近浏览 -->
      <div
        v-if="recentViews.length && !searchText && activeCategory === 'all'"
      ...
      <div v-loading="loading" class="card-area">
        <div v-if="items.length" class="card-grid">
          ...
          >
            ...
            <div class="k-card-tags">
              <el-tag
                v-for="dm in item.device_models"
                :key="dm"
                size="small"
                type="success"
                effect="plain"
            ...
            <div class="k-card-footer">
              <div class="k-card-meta">
                ...
                <span class="k-card-views" v-if="item.view_count"
                  ><el-icon><View /></el-icon> {{ item.view_count }}</span
                >
              </div>
        ...
        <el-empty
          v-else-if="!loading"
          :description="
            searchText || selectedDevice || selectedFault
              ? '没有匹配的知识条目'
              : '知识库暂为空'
          "
        >
...
const recentViews = ref(
  JSON.parse(localStorage.getItem("recent_knowledge_views") || "[]"),
);
...
function goDetail(id) {
  ...
  let recents = JSON.parse(
    localStorage.getItem("recent_knowledge_views") || "[]",
  );
  if (item) {
...
function removeRecent(id) {
  ...
  localStorage.setItem(
    "recent_knowledge_views",
    JSON.stringify(recentViews.value),
  );
}
</script>
